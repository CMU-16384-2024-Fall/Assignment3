...
\begin{questions}
    ...
    transforms.  Number the frames along the chain the way you did in the last
    assignment --- a rotation at each joint, a translation along each link --- so
    that $\H{2}{0}$ is the frame at the far end of link 1 (the elbow) and
    $\H{4}{0}$ is the frame at the far end of link 2 (the end effector), both
    ...
    ever touch a robot.  With \verb!ROBOT_IP! unset, run:
    \begin{verbatim}
    python replay.py square.csv
\end{verbatim}
    Two windows open: the simulated xArm7 in a browser tab (its address is also
    printed as \verb!Meshcat: ...! when the script starts), and a top-down plot
    ...
    to your right.  In it you should see
    \begin{itemize}
      \item the blue stick figure lying on top of the simulated arm, with its
            elbow at the simulated arm's elbow;
      \item the solid black path the end effector traces landing on the dotted
            orange path it was asked to follow;
      \item the green arrow pointing along the direction of travel, growing when
            the arm moves faster and shrinking to nothing when it stops.
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item The path is right but the arrow points the wrong way, or has wildly
            the wrong length --- \verb!jacobian_RR!.
      \item \verb!a 0.213 rad jump at sample 57! --- the samples reaching the arm
            are out of order, in degrees rather than radians, or $\th[1]$ and
            $\th[2]$ the wrong way round.
    \end{itemize}
    ...
    \begin{itemize}
      % TODO: exact power-on sequence (controller box switch, wait time, what the
      % status light should look like) --- confirm with the lab staff.
      ...
      % TODO: say exactly where the e-stop is on the workstation.
      ...
      \item \textbf{The motors stay on.} When a script exits, the arm is left
            stopped but still energized and holding its position.  That is
            deliberate: switching the motors off would let it fall under its own
            weight.
    \end{itemize}
    \end{tcolorbox}
    \newpage
    ...
    \begin{enumerate}
      % TODO: confirm whether ROBOT_IP is already set on the lab machines; if it
      % is not, give students the per-station address here.
      % TODO: confirm which editors are installed on the lab machines.
...
python record.py my-path.csv
    ...
    \paragraph{Replaying It} Play the recording back on the arm with the same
    script you tested in simulation:
    \begin{verbatim}
    python replay.py my-path.csv
\end{verbatim}
